import 'package:svg_to_vector_drawable/shape_converter.dart';
import 'package:test/test.dart';
import 'package:xml/xml.dart';

void main() {
  group('ShapeConverter.lineToPath', () {
    test('converts a line', () {
      final element = XmlDocument.parse(
        '<line x1="0" y1="80" x2="100" y2="20" />',
      );
      final result = ShapeConverter.lineToPath(element.getElement('line')!);
      expect(
  ...
  group('ShapeConverter.rectToPath', () {
    test('converts a rect with no x & y', () {
      final element = XmlDocument.parse('<rect width="100" height="100" />');
      final result = ShapeConverter.rectToPath(element.getElement('rect')!);
      expect(
        result.toXmlString(pretty: true),
  ...
  group('ShapeConverter.circleToPath', () {
    test('converts a circle', () {
      final element = XmlDocument.parse('<circle cx="50" cy="50" r="50"/>');
      final result = ShapeConverter.circleToPath(element.getElement('circle')!);
      expect(
        result.toXmlString(pretty: true),
  ...
  group('ShapeConverter.ellipseToPath', () {
    test('converts an ellipse', () {
      final element =
          XmlDocument.parse('<ellipse cx="100" cy="50" rx="100" ry="50" />');
      final result =
          ShapeConverter.ellipseToPath(element.getElement('ellipse')!);
      expect(
        result.toXmlString(pretty: true),
  ...
  group('ShapeConverter.polygonToPath', () {
    test('converts a polygon', () {
      final element =
          XmlDocument.parse('<polygon points="0,100 50,25 50,75 100,0" />');
      final result =
          ShapeConverter.polygonToPath(element.getElement('polygon')!);
      expect(
        result.toXmlString(pretty: true),
    ...
    test('converts a polyline', () {
      ...
      final result = ShapeConverter.polygonToPath(
